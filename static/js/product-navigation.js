...
function navigateToProduct(productId) {
  // If we're already on a product page, just update the content
  // if (!isHomePage) {
  //   updateProductDetail(productId);
  //   return;
  // }
  
  // Otherwise, fetch the product detail page template
  fetch('/products/?id=' + productId)
    ...
    .then(html => {
      ...
      setupProductNavigation();
      initializeCommentSystem(); // Commented out as the function is not defined
...
function updateSuggestedProducts(currentProductId) {
  ...
  suggested.forEach(product => {
    ...
    // Add click event for dynamic navigation
